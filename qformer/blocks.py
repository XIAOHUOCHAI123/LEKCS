...
class TextBlock(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.dropout = dropout
        #queries
        self.query_tokens = nn.Parameter(torch.randn(1, bconfigs["num_query_token"], 1024))
        #self.query_tokens.data.normal_(mean=0.0, std=encoder_config.initializer_range)

        self.attn = MultiQueryAttention(1024, heads)
        self.s_att = nn.MultheadAttention(d_model=1024,nhead=4,dropout=0.1)
        #self.c_att = nn.MultheadAttention(d_model=1024,nhead=4,dropout=0.1)
